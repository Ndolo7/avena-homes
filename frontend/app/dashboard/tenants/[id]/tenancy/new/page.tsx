...
export default function NewTenancyPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/properties/units/?status=vacant&page_size=200`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((r) => r.json())
      .then((data: PaginatedResponse<Unit>) => setUnits(data.results))
      .catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/tenants/tenancies/`, {
        method: "POST",
  ...
  return (
    <div className="max-w-xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Create Tenancy</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Assign tenant to a unit and set lease terms.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          {units.length === 0 ? (
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (selectedUnit?.base_rent) {
                  setRentAmount(String(selectedUnit.base_rent));
                  setDepositAmount(String(selectedUnit.base_rent));
                } else {
                  setRentAmount("");
                  setDepositAmount("");
                }
            ...
            >
              ...
              {units.map((u) => (
                <option key={u.id} value={u.id}>
                  Unit {u.unit_number} — KES {Number(u.base_rent).toLocaleString()}/mo
                </option>
              ))}
            </select>
          )}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="rent_amount" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
              Monthly rent (KES) <span className="text-rose-500">*</span>
            </label>
            <input id="rent_amount" name="rent_amount" type="number" required placeholder="25000"
          ...
          <div>
            <label htmlFor="deposit_amount" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
              Deposit (KES) <span className="text-rose-500">*</span>
            </label>
            <input id="deposit_amount" name="deposit_amount" type="number" required placeholder="50000"
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <select id="billing_cycle" name="billing_cycle" required className="w-full px-4 py-2.5 rounded-xl text-sm border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-colors">
        ...
        <div>
          ...
          <input 
            ...
            className="w-full px-4 py-2.5 rounded-xl text-sm border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-colors cursor-pointer" 
          />
        </div>
